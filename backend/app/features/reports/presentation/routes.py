...
@router.post("/generate", response_model=OperationReportDTO, status_code=status.HTTP_201_CREATED)
async def generate_report_route(
    report_type: str = "daily",
    service: ReportsService = Depends(get_reports_service),
    current_user: UserDTO = Depends(organizer_access)
):
    return await service.generate_report(report_type=report_type, created_by_id=current_user.id)
