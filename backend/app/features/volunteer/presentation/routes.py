...
from app.features.volunteer.presentation.schemas import VolunteerTaskCreateSchema, VolunteerTaskUpdateSchema
...
async def get_tasks_route(
    ...
    return await service.list_tasks(user_role=current_user.role, user_id=current_user.id)

@router.post("/tasks", response_model=VolunteerTaskDTO, status_code=status.HTTP_201_CREATED)
